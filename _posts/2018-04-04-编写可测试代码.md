...
我们都有过这样的经验：写几行Javscript代码来实现一个小功能，然后随着需求变化变成了十几行，然后二十几行，然后更多。在这个过程中，函数的参数越来越多，条件判断的分支也越来越多。然后有一天，代码出了问题，我们开始从一堆杂乱的代码中去找bug。

当我们要求客户端代码执行越来越多的功能时 --实际上，现在整个应用程序很大一部分都是在客户端实现的-- 有两件事是可以确定的。1.我们无法通过敲敲键盘， 点点鼠标， 就确保功能无误， 而要通过自动化测试确保代码的正确性。2. 为了让代码便于自动化测试，或许我们需要稍微改变下写代码的方式。


虽然都知道自动化测试很好，但是现在大部分都只能实现集成测试。集成测试可以确保系统整体运行正常，但是无法确保单元功能正常。

这就需要我们编写单元测试了。而编写单元测试会一直纠结直到我们开始*编写可测试的JavaScript*。

##单元VS集成： 区别在哪？

编写集成测试代码通常都非常直接：我们写代码来描述用户应该输入什么，然后他又应该看见什么。
[Selenium](http://docs.seleniumhq.org/)是一个流行的自动化浏览器测试工具。[Capybara](https://github.com/jnicklas/capybara)让Ruby下的Selenium非常好用。 针对许多其他的语言也有其专门的工具。
...
与集成测试关注的是用户和程序的交互相比，单元测试仅仅关注小段代码的行为：
> 当我传递给一个函数一定的参数时，我有没有获得期望的返回值？

用传统方式写的代码可能会非常难写对应的单元测试用例-- 而且很难维护，调试，扩展。但是如果我们写代码时已经构思好了将要写的单元测试， 我们可能会发现不仅编写单元测试变得很容易，代码质量也会随之提高！
...
虽然我们可以写集成测试--我们也应该写--写单元测试却非常难。在功能性测试中，我们作出诸如“当用户搜索关键词，她应当看见正确的结果”的断言，却无法更加具体。如果哪里出了问题，就只能跟踪代码找到错误的地方，而功能性测试在这时候没什么用。
...
* 保持目标代码的简洁，这有助于我们保持测试简单及代码可读
* 用构造函数来创建实例。这让就可以为了测试的需求“干净地”复制每段代码。

首先我们要先找到从哪里开始分解代码。有三块用来做展现和交互： 搜索表单，搜索结果，以及“Like框”

<img src="http://d.alistapart.com/375/appviews.png" alt="Application Views">

同样会有一块代码用来从服务器取数据，以及另外一块用来将其他模块组织在一起。

先看看代码中最简单的Like框。在初始的版本中，这些代码用来更新Like框： 
...
这段代码提供了一个用来创建新Like框实例的构造函数。这个实例会有个`.add()`方法用来添加搜索结果。这样我们就可以给它写单元测试了。
...
没那么难吧？ 我们使用[Mocha](http://visionmedia.github.io/mocha/)作为测试框架， [Chai](http://chaijs.com/)作为测试库。 Mocha提供了`test`和`setup`函数; Chai提供了assert函数。还有许多其他可选的测试框架和库， 但这里为了解说， 这两个库够了。在实践中可以选择最适合自己和项目的: 除了Mocha， [Qunit](http://qunitjs.com/)很流行， [Intern](http://theintern.io/)显示了很多潜力。
...
-这比原来的实现简洁多了， 因为我们将`document`作为一个传递消息的枢纽，组件之间就可以在不知道彼此结构的情况下进行通信。（在一个商业的APP中，我们一般会使用诸如[Backbone](http://backbonejs.org/)或者是[RSVP](https://github.com/tildeio/rsvp.js)这样的框架来管理事件，这里我们用document来保持简洁。）我们也将复杂的操作--例如从被喜欢的搜索结果中找出人名--交给SearchResults对象，而非让它把代码结构整的乱七八糟。最好的是：现在我们同样可以为SearchResults编写单元测试了：
...
* 我们不想直接和服务器进行交互--否则就要重新进行集成测试， 而且我们都是负责任的程序员， 所以服务器的代码肯定被测试过了，是吧？ 相应地， 我们“模仿”和服务器的交互，通过[Sinon](http://sinonjs.org/)。
...
比获得简洁的代码更重要的是， 现在我们完整的测试了代码。  这样我们就可以放心大胆地重构代码而不用怕会破坏之前写好的成果。 当出现新情况时， 甚至可以编写新的
测试用例， 然后再写相应的代码。
...
这篇文章讨论了Javascript测试的一些基本知识。但是如果想了解更多，可以参考：

* [我的报告](http://lanyrd.com/2012/fullfrontal/sztqh/), 来自2012年伦敦Brighton的Full Frontal会议。
* [Grunt](http://gruntjs.com/)用来进行自动化测试和许多其他任务的工具。
* [编写可测试的Javascript代码](http://www.amazon.com/TestDriven-JavaScript-Development-Developers-Library/dp/0321683919/ref=sr_1_1?ie=UTF8&qid=1366506174&sr=8-1&keywords=test+driven+javascript+development) 作者Christian Johansen， Sinon库的作者。这是一本关于JS测试的紧凑但信息丰富的书。
